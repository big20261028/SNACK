...
<section>
	<h2 class="subTitle">팔로우 작가의 글</h2>
	...
	<form id="flist" name="flist" method="post" action="flist.do">
	...
	<div id="pageNumber">
		<c:if test="${ startbk > 10 }"> 
			<a href="flist.do?pageno=${startbk - 1}&listFilter=${ searchVO.listFilter }&searchFilter=${ searchVO.searchFilter }&keyword=${ searchVO.keyword }">◀</a>
		</c:if> 
	 	<c:forEach var="page" begin="${startbk}" end="${endbk}">
	 		<a href="flist.do?pageno=${page}&listFilter=${ searchVO.listFilter }&searchFilter=${ searchVO.searchFilter }&keyword=${ searchVO.keyword }">${page}</a>
	 	</c:forEach>
	 	<c:if test="${ endbk < maxpage }"> 
	 		<a href="flist.do?pageno=${endbk + 1}&listFilter=${ searchVO.listFilter }&searchFilter=${ searchVO.searchFilter }&keyword=${ searchVO.keyword }">▶</a>
	 	</c:if>
	</div><br>
</section>
