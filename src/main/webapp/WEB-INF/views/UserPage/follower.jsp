...
<script>
	...
	function followerList(page) {
		console.log("followerList 호출됨");
	    $.ajax({
	        url: "followerList.do", // 현재 요청을 보낼 URL (컨트롤러의 /UserPage/follow.do 매핑)
	        type: "post", 
	        headers: {
	        ...
	        success: function(data) {
	           	...
	           	if (data.list == null || data.list.length === 0) {
	                ...
	                $.each(data.list, function(index, follow) {
	                    ...
	                    html += '<a href="/control/UserPage/userpage.do?usernum=' + follow.fromnum + '">' + follow.userid + '</a>';
	                    html += '</div>';
	                    if(${ sessionScope.login != null } && follow.fromnum != "${ sessionScope.login.usernum }" )
                    	{
                    		html += '<div class="followBox whiteBtn followSub" id="followSub' + follow.fromnum + '" onclick="subscribe(${ sessionScope.login.usernum },' + follow.fromnum + ');"></div>';
                    	}
	                    html += '</div>';
	                ...
	                if (data.startbk > 10) {
	                	html += '<a href="javascript:followerList(' + (data.startbk - 1) + ')">◀</a>';
		            }
		            for (var i = data.startbk; i <= data.endbk; i++) {
		            	html += '<a href="javascript:followerList(' + i + ')">' + i + '</a> ';
		            }
		            if (data.endbk < data.maxpage) {
		            	html += '<a href="javascript:followerList(' + (data.endbk + 1) + ')">▶</a>';
		            }
		            	html += '</div>';
    ...
    <section>
      ...
      <h2 class="subTitle">
	      ...
	      <c:if test="${ sessionScope.login == null || sessionScope.login.usernum != userVO.usernum }">
	    	${ userVO.userid } (을)를 구독하는 ${ total } 명  
	      </c:if>
      </h2>
      <div class="box" id="followContentBox">
      	<c:forEach var="item" items="${ followerList }">
   			<div class="marginB marginT">
	            <div class="positionR">
		            ...
		            <div class="followBox" id="followId"><a href="/control/UserPage/userpage.do?usernum=${ item.fromnum }">${ item.userid }</a></div>
		            <c:if test="${ sessionScope.login != null }">
		            	<c:if test="${ sessionScope.login.usernum != item.fromnum }">
			            	<div class="followBox whiteBtn followSub" id="followSub${ item.fromnum }"
			            	onclick="subscribe(${ sessionScope.login.usernum },${ item.fromnum });"></div>
		            	</c:if>
		            </c:if>
	        	</div>
        ...
        <div id="pageNumber">
			<c:if test="${ startbk > 10 }">
				<a href="javascript:followerList(${startbk - 1})">◀</a>
			</c:if>
			<c:forEach var="page" begin="${startbk}" end="${endbk}">
				<a href="javascript:followerList(${page});">${page}</a>
			</c:forEach>
			<c:if test="${ endbk < maxpage }">
				<a href="javascript:followerList(${endbk + 1});">▶</a>
			</c:if>
		</div>
      </div>
      <!-- 이곳에 컨텐츠 입력 -->
